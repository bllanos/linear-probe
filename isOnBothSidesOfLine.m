...
if all(size(l) == [1 3])
    ...
    [~, signs] = cellfun(@distanceToHomog2DLine, p, l_rep, 'UniformOutput', false);
elseif all(size(l) == [2 2])
    % Line defined by two endpoints
    [~, signs] = cellfun(@distanceToLine, p, l_rep, 'UniformOutput', false);
else
    error('Unknown form of the second input argument `l` or `line_points`.')
end

p_filter = cellfun(@isOnBothSides, signs, 'UniformOutput', true);
end
